"""Parse VCD files, compare RTL vs synthesised netlist, and save waveform PNGs.

All-modes variant: tb_spi_slave.v runs the read/write/read sequence once per
SPI mode (0..3) with a DUT reset in between, so both VCDs cover 4 × 1120 ns.
"""
import re
import matplotlib
matplotlib.use("Agg")
...
print("Parsing tb_spi_slave_synth.vcd (synth)...")
_, syn = parse_vcd("tb_spi_slave_synth.vcd")

T_END      = 4500
MODE_SPAN  = 1120   # ns per mode window (reset + 3 transactions)
MODE_COLS  = ['green', 'orange', 'steelblue', 'purple']
...
print("\n=== B2B Signal Comparison at end of simulation (after mode 3) ===")
all_match = True
...
# ---------------------------------------------------------------------------
# Plot 1: RTL waveform — all four modes
# ---------------------------------------------------------------------------
SIG_LIST = [
    ('rst_n', 1, False),
    ('cpol',  1, False),
...
fig, axes = plt.subplots(len(SIG_LIST), 1, figsize=(16, 9), sharex=True)
fig.suptitle("SPI Slave (all-modes) — RTL Simulation: READ/WRITE/READ in SPI modes 0–3",
             fontsize=11, y=0.99)
...
for ax, (name, width, is_bus) in zip(axes, SIG_LIST):
    ...
    ax.step(t_arr, norm, where='post', color=CLR_RTL, linewidth=1.0)
    ax.fill_between(t_arr, 0, norm, step='post', alpha=0.15, color=CLR_RTL)

    if is_bus:
        prev_v = None
        for t_v, v_v in entries:
            if v_v != prev_v:
                ax.text(t_v + 4, 0.5, f'0x{v_v:02X}', fontsize=6,
                        va='center', ha='left', color=CLR_RTL)
                prev_v = v_v

# Mode window shading and labels
for m in range(4):
    t0, t1 = m * MODE_SPAN, (m + 1) * MODE_SPAN
    for ax in axes:
        ax.axvspan(t0, t1, alpha=0.05, color=MODE_COLS[m])
    axes[0].text((t0 + t1) / 2, 1.12,
                 f"Mode {m} (CPOL={m >> 1} CPHA={m & 1})",
                 fontsize=8, ha='center', color=MODE_COLS[m])
...
# ---------------------------------------------------------------------------
# Plot 2: B2B overlay — RTL vs Synth (same TB run on both)
# ---------------------------------------------------------------------------
B2B_SIGS = [
    ('ssel',     1, False),
    ('miso',     1, False),
...
fig2, axes2 = plt.subplots(len(B2B_SIGS), 1, figsize=(16, 7), sharex=True)
fig2.suptitle("SPI Slave (all-modes) — B2B Comparison: RTL vs Synthesised Netlist, modes 0–3",
              fontsize=11, y=0.99)
...
for ax, (name, width, is_bus) in zip(axes2, B2B_SIGS):
    ...
    for label, wave_dict, col, lw, ls in [
        ('RTL',   rtl, CLR_RTL,   2.0, '-'),
        ('Synth', syn, CLR_SYNTH, 1.2, '--'),
    ]:
        if name not in wave_dict: continue
        _, entries = wave_dict[name]
        t_arr, v_arr = waveform_steps(entries, T_END)
        norm = v_arr / maxv if maxv > 0 else v_arr
        ax.step(t_arr, norm, where='post', color=col, linewidth=lw,
...
for m in range(4):
    ...
    for ax in axes2:
        ax.axvspan(t0, t1, alpha=0.05, color=MODE_COLS[m])
    axes2[0].text((t0 + t1) / 2, 1.20,
                  f"Mode {m}", fontsize=8, ha='center', color=MODE_COLS[m])
